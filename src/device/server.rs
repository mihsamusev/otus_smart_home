...
impl SmartSocketServer {
    ...
    pub fn listen(&mut self) {
        println!("Listening on {}", &self.listener.local_addr().expect("Couldnt get local addr"));
        for stream in self.listener.incoming() {
            ...
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    handle_smart_socket(stream, socket_ref).unwrap_or_else(|_| eprintln!("{} disconnected", client_addr));
                });
            }
        }
    }
...
fn handle_smart_socket(mut stream: TcpStream, device: Arc<Mutex<SmartSocketDevice>>) -> Result<(), io::Error> {
